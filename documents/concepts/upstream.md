...
- **上游服务器 (`servers`)**: 就像是办公室里的一个个"组员"。组员可以是 DoH（URL）或传统 DNS（`IP:端口`）。
...
- 对于 `scheme: doh`：内部维护一个**共享的 HTTP 客户端连接池**，高效复用与上游服务器的 TCP/TLS 连接。
- 对于 `scheme: dns`：内部会复用与上游服务器的 TCP 连接，并支持在 TCP 请求失败后按需重连（见 `dns_client.tcp_reconnect`）。
