...
**Load Ants** 是一款专为提升网络隐私、安全与灵活性而生的高性能、多功能 DNS 代理服务。支持 **RFC 8484** 和 **Google DoH**（DNS over HTTPS）标准。

它支持 UDP/TCP/DoH 入站，并可将请求按路由策略转发到上游解析器：**DoH** 或 **传统 DNS（UDP/TCP）**。同时提供智能路由能力，并支持 DoH 代理转发。
...
传统的 DNS 查询以明文形式传输，这意味着你的上网记录很容易被窃取、监控甚至篡改。Load Ants 旨在解决这些根本性问题：

- 🔒 **强化隐私保护**：可通过 DoH 加密 DNS 请求，有效防止网络运营商或中间人窥探你的浏览历史。
...
- 🔄 **多协议入站**：同时支持 `UDP/53`、`TCP/53` 与 DoH 入站。
- 🌍 **DoH 代理**：支持 DoH 代理，可以作为 DoH 服务端使用。支持 `GET` 和 `POST` 方法。
- 🧠 **智能路由决策**：可根据精确域名、通配符、正则表达式等多种模式，将查询转发到不同上游或直接拦截。
- 🌐 **灵活上游管理**：上游支持 DoH 与传统 DNS（UDP/TCP），并支持分组管理与多种负载均衡策略（轮询、加权、随机）。
- ⚡ **高性能缓存**：内置正向与负向缓存，有效降低延迟，减少对上游服务器的请求压力。
- 📜 **远程规则列表**：支持从 URL 动态加载规则列表（如广告拦截、代理规则），让策略管理更便捷。
- 💾 **远程规则快照恢复**：自动保存成功拉取的远程规则快照，启动时若远程源失败，可用最近快照恢复。
- 🎲 **0x20 大小写随机化**：针对传统 DNS 上游随机化查询名大小写，防御响应伪造攻击。
- 🧹 **污染应答过滤**：通过 `deny_answers` 过滤响应中的已知污染 IP 段（CIDR 格式）。
- 🔀 **请求合并**：并发的相同查询只向上游发出一次请求，共享同一解析结果。
- 🚦 **速率限制**：支持全局与单 IP 两级请求速率限制。
...
- **开发者与技术爱好者**：需要在本地环境测试或使用 DoH，并精细控制 DNS 行为。
...
- [➡️ 快速上手](./getting-started/index.md)
