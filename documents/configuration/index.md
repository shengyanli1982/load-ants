...
欢迎来到 Load Ants 的配置指南。本章逐一拆解 `config.yaml` 中的配置项，帮你配出完全符合自己需求的 DNS 服务。
...
> ✨ **专家提示**：
> 为了保证可移植性和清晰性，强烈建议始终将 `config.yaml` 文件与 Load Ants 程序放在同一个目录下，并用 `-c ./config.yaml` 显式加载。
...
2. **运行时语义校验（启动阶段）**：
    ...
    - 只要存在任何 `forward` 规则：必须配置 `upstream_groups`，且所有 `target` 必须引用已存在的上游组名（同时，上游组名称必须唯一）。
...
Load Ants 的配置文件使用 [YAML](https://yaml.org/) 格式。YAML 的基本规则如下：

- **缩进**：YAML 使用缩进表示数据的层级关系，建议 **2 个空格**作为一级缩进。
...
一个完整的 `config.yaml` 文件由以下几个顶层配置块组成。你可以根据自己的需求，选择性地启用或忽略非必选的配置块。
...
# 上游组（upstream group，条件必选：只要存在任何 forward 规则就必须配置）
upstream_groups:
    # ...
...
- [`admin`](./server.md#admin-服务器)：配置 Admin（管理）服务器的监听地址。
- [`cache`](./cache.md)：配置内置 DNS 缓存的行为。
- [`http_client`](./http-client.md)：定义全局 HTTP 客户端的行为，影响 DoH（DNS over HTTPS）上游与规则下载。
