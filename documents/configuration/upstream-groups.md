...
`upstream_groups` 定义了你的 DNS 查询最终将被发送到哪些上游解析器（**DoH（DNS over HTTPS）** 或 **传统 DNS（UDP/TCP）**），以及如何在高可用、高性能和高隐私之间取得平衡。

本配置项是一个**列表**，你可以定义多个独立配置的上游组（upstream group）。

上游管理和负载均衡策略背后的设计理念详见 [**核心概念：上游服务器管理**](../concepts/upstream.md)。
...
| 参数       | 类型   | 描述                                                                                                                                                                                            | 默认值 | 是否必填 |
| :--------- | :----- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----- | :------- |
| `name`     | 字符串 | 为这个上游组指定一个唯一的名称。该名称将在路由规则（`static_rules` 或 `remote_rules`）的 `target` 字段中被引用。                                                                                | -      | **是**   |
| `scheme`   | 字符串 | 上游组类型。可选值：`doh`（DoH 上游）或 `dns`（传统 DNS 上游，UDP/TCP）。为兼容旧配置，也支持别名字段 `protocol`。                                                                              | `doh`  | 否       |
| `strategy` | 字符串 | 定义该组内服务器的负载均衡策略。可选值为：`roundrobin`（轮询）、`weighted`（加权）、`random`（随机）。                                                                                          | -      | **是**   |
...
| `proxy`    | 字符串 | （可选，仅 `scheme: doh`）为**整个组**的服务器指定一个出站代理。所有发往该组内服务器的 DoH 请求都将通过此代理。支持 `http`、`https` 协议，例如 `http://user:pass@host:port`。                   | -      | 否       |
| `tls_verify` | 布尔 | （可选）是否校验 DoH 上游的 TLS 证书。`false` 跳过证书校验；不配置时启用校验。                                                   | -      | 否       |
| `deny_answers` | 列表 | （可选）已知污染 IP 段列表（CIDR 格式，例如 `["1.1.1.1/32", "10.0.0.0/8"]`），用于过滤上游响应中 `A`/`AAAA` 记录里的污染 IP；若过滤后 `A`/`AAAA` 记录全部被移除，则返回 `SERVFAIL`。 | `[]`   | 否       |
| `case_randomization` | 布尔 | （可选）是否启用 0x20 大小写随机化防响应伪造，仅对 `scheme: dns` 的上游组生效。如果上游响应不保留查询中的大小写，启用后会导致验证失败。 | `false` | 否      |
| `case_randomization_strict` | 布尔 | （可选）0x20 大小写随机化验证的严格模式，仅在 `case_randomization: true` 时生效。`true`：UDP 响应验证失败时丢弃该响应并回退 TCP 重发；`false`：仅记录警告日志，仍接受 UDP 响应。 | `false` | 否      |
...
| `weight`       | 整数   | （可选）服务器的权重，仅在组的 `strategy` 为 `weighted` 时生效。权重越高的服务器，被选中的概率越大。有效范围 `1-65535`。                                               | `1`         | 否       |
...
| `weight` | 整数   | （可选）服务器的权重，仅在组的 `strategy` 为 `weighted` 时生效。有效范围 `1-65535`。          | `1`    | 否       |
...
| 参数       | 类型 | 描述                                                                               | 默认值 | 是否必填                    |
| :--------- | :--- | :--------------------------------------------------------------------------------- | :----- | :-------------------------- |
| `attempts` | 整数 | 最大重试次数（不含首次请求），有效范围 `1-100`。例如 `3` 表示首次请求失败后最多再重试 3 次。 | -      | **是**（若 `retry` 块存在） |
| `delay`    | 整数 | 退避基准延迟（秒），有效范围 `1-120`。实际重试间隔会随退避策略增长。               | -      | **是**（若 `retry` 块存在） |

> **注意**：除 `retry` 外，转发层还存在换服务器重试：当请求组内某台服务器失败时，会自动切换到同组的其他服务器重试，最多尝试次数等于该组的服务器数量。
...
**目标**：主要使用一个高性能的服务器，当该服务器不可用时，自动切换到备用服务器。
...
**目标**：连接需要认证、并且通过代理访问的私有 DoH 服务（以 NextDNS 为例）。

```yaml
upstream_groups:
    - name: "private_nextdns"
      scheme: "doh"
      strategy: "roundrobin" # 只有一个服务器时，策略的取值不影响结果
      # 为整个组配置代理
      proxy: "http://127.0.0.1:7890"
      servers:
