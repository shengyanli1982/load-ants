...
路由规则是 Load Ants 的"大脑"，它决定了如何处理每一个接收到的 DNS 查询。通过组合使用静态规则（`rules.static`）和远程规则（`rules.remote`），你可以构建出强大而灵活的 DNS 流量管理策略。
...
| `upstream` | 字符串 | 目标上游组的名称。仅在 `action` 为 `forward` 时需要。此名称必须与 `upstreams` 中定义的某个组的 `name` 相对应。 | -      | **是** (若 `action` 为 `forward`) |
...
- 当 `action: "forward"`：必须提供 `upstream`，且该值必须引用一个已存在的 `upstreams[].name`；另外，上游组名称必须唯一。
...
与上游组（`upstreams`）不同，`rules.remote` 拥有自己独立的网络配置，用于控制规则文件的下载行为。
...
> - `rules.remote` 的下载会复用全局 `http` 的超时/连接池配置，但其 `proxy`/`auth`/`retry` 为规则源独享配置。

> ✨ **专家提示**:
>
> **独立的网络配置**：`rules.remote` 拥有独立的 `proxy`, `auth`, `retry` 配置，是因为规则文件所在的服务器网络环境可能与你日常使用的 DoH 服务器完全不同。例如，某个规则列表可能托管在需要特定代理才能访问的 GitHub Gist 上，而你的 DoH 查询则希望直连。这种独立性提供了极大的灵活性。
...
rules:
    static:
        ...
        - match: "wildcard"
          patterns: ["*"]
          action: "forward"
          upstream: "privacy_first" # 假设你已定义了一个注重隐私的上游组
...
```yaml
# 注意：以下 upstreams 应在你的主配置文件的顶层定义。
# 这里为了示例清晰而放在一起。
