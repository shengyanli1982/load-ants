...
`dns_client` 配置块定义 **传统 DNS 上游（UDP/TCP，端口 53）** 的全局客户端行为。

该配置块仅在某个上游组设置了 `scheme: dns` 时生效；DoH（DNS over HTTPS）上游与规则下载仍由 [`http_client`](./http-client.md) 控制。
...
dns_client:
    ...
    tcp_reconnect: true # TCP 请求失败后丢弃连接，立即重建并重试一次
```
...
| `tcp_reconnect` | 布尔 | TCP 请求失败后是否立即重建连接并重试本次请求。无论取值如何，失败的连接都会被丢弃：`true` 立即重建连接并重试本次请求一次；`false` 本次请求直接失败（下次请求会建立新连接）。 | `true` | 否 |
| `idle_connection_timeout` | 整数 | 空闲连接回收超时（秒），有效范围 `5-1800`。连接池中空闲超过该时长的 TCP/UDP 连接会被定期回收。 | `30` | 否 |
| `max_tcp_connections` | 整数 | TCP 连接池容量上限（所有上游共享），有效范围 `1-65535`。达到上限时淘汰最老的连接。 | `256` | 否 |
...
> - UDP 的超时或 I/O 错误 **不会** 自动触发 TCP 重试；只有在收到 UDP 响应且 `TC=1` 时才会触发 UDP→TCP 回退（或你显式设置 `prefer_tcp=true`）。
