...
`dns` 配置块用于定义 **传统 DNS 上游（UDP/TCP，端口 53）** 的全局客户端行为。

它仅在你的某个上游组设置了 `protocol: dns` 时生效；DoH 上游与规则下载仍由 [`http`](./http-client.md) 控制。
...
| :---------------- | :--- | :------------------------------------------------------------------------------------------------------------------ | :----------------- | :------------------------ |
| `connect_timeout` | 整数 | TCP 建立连接的超时时间（秒），有效范围 `1-120`。仅对 TCP 生效。                                                     | `2`                | **是**（若 `dns` 块存在） |
| `request_timeout` | 整数 | 单次上游请求等待超时时间（秒），有效范围 `1-1200`。当触发 UDP→TCP 重试时，UDP 与 TCP 各自独立计时。                 | `3`                | **是**（若 `dns` 块存在） |
...
### 行为说明（与 `upstreams[].protocol: dns` 的关系）

要启用传统 DNS 上游，请在上游组中设置 `protocol: dns`，并在 `endpoints` 中使用 `addr`：
...
> 说明：
>
> - `protocol: dns` 的上游组 **不支持** `proxy` 与 `retry`（会触发配置校验错误）。
...
- [➡️ 了解上游组配置（protocol=doh|dns）](./upstream-groups.md)
