...
`http` 配置块用于定义全局的 HTTP 客户端行为。这里的设置将影响所有出站 HTTP 请求，包括：

- 上游组（`upstreams`）发出的 DoH (DNS-over-HTTPS) 请求
- 远程规则（`rules.remote`）下载规则文件时的 HTTP 请求

> 如果你使用了传统 DNS 上游（`upstreams[].protocol: dns`），其 UDP/TCP 行为由 [`dns`](./dns-client.md) 单独控制，不受 `http` 影响。
...
| :---------------- | :----- | :---------------------------------------------------------------------------------------------------- | :----------------- | :------------------------- |
| `connect_timeout` | 整数   | 建立新连接（TCP 握手）的超时时间（秒），有效范围 `1-120`。                                            | `3`                | **是**（若 `http` 块存在） |
| `request_timeout` | 整数   | 单次请求的总超时时间（秒，从建立连接到接收完整响应），有效范围 `1-1200`。                             | `5`                | **是**（若 `http` 块存在） |
...
| `user_agent`      | 字符串 | `User-Agent` 请求头。你可以将其设置为任意值，或不设置（不设置时由 HTTP 客户端保持默认行为）。         | （不设置）         | 否                         |
