...
# 构建静态 Server 二进制
RUN cargo build --bin loadants --target x86_64-unknown-linux-musl --release
...
# 从构建阶段复制编译好的二进制文件
COPY --from=builder /build/target/x86_64-unknown-linux-musl/release/loadants /app/loadants
