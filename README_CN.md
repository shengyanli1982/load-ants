...
| tcp_timeout | 整数   | 10             | TCP 连接空闲超时（秒） | 1-3600              |
...
| min_ttl      | 整数   | 60     | 最小 TTL（秒），会覆盖原始响应中更小的 TTL 值      | 1-86400    |
| max_ttl      | 整数   | 3600   | 所有缓存条目的最大生存时间上限（秒）               | 1-86400    |
| negative_ttl | 整数   | 300    | 负向缓存 TTL（秒），用于缓存错误、不存在域名等响应 | 1-86400    |
...
| 参数     | 类型   | 默认值 | 描述                                     | 有效范围                           |
| -------- | ------ | ------ | ---------------------------------------- | ---------------------------------- |
| name     | 字符串 | -      | 组名称 (必填, 需唯一)                    | 非空字符串                         |
| strategy | 字符串 | -      | 负载均衡策略 (必填)                      | "roundrobin", "weighted", "random" |
| servers  | 数组   | -      | 此组中的 DoH 服务器列表 (必填, 至少一个) | -                                  |
...
| 参数         | 类型   | 默认值    | 描述                                                               | 有效范围                     |
| ------------ | ------ | --------- | ------------------------------------------------------------------ | ---------------------------- |
| url          | 字符串 | -         | DoH 服务器 URL (必填)                                              | 有效的 HTTP(S) URL，包含路径 |
| weight       | 整数   | 1         | 权重 (仅当组策略为 `weighted` 时有效)                              | 1-65535                      |
| method       | 字符串 | "post"    | DoH 请求方法 (GET 或 POST)                                         | "get", "post"                |
...
| 参数     | 类型 | 默认值 | 描述           | 有效范围 |
| -------- | ---- | ------ | -------------- | -------- |
| attempts | 整数 | 3      | 重试次数       | 1-100    |
| delay    | 整数 | 1      | 初始延迟（秒） | 1-120    |
...
| 参数     | 类型   | 默认值 | 描述                                             | 有效范围                     |
| -------- | ------ | ------ | ------------------------------------------------ | ---------------------------- |
| match    | 字符串 | -      | 匹配类型 (必填)                                  | "exact", "wildcard", "regex" |
| patterns | 数组   | -      | 匹配模式列表 (必填, 至少一个模式)                | 非空字符串数组               |
| action   | 字符串 | -      | 路由动作 (必填)                                  | "forward", "block"           |
...
| 参数     | 类型   | 默认值  | 描述                                                 | 有效范围                             |
| -------- | ------ | ------- | ---------------------------------------------------- | ------------------------------------ |
| type     | 字符串 | "url"   | 规则类型，目前仅支持 "url"                           | "url"                                |
| url      | 字符串 | -       | 远程规则文件的 URL (必填)                            | 有效的 HTTP(S) URL                   |
| format   | 字符串 | "v2ray" | 规则文件格式                                         | "v2ray" (未来可能支持 "clash" 等)    |
| action   | 字符串 | -       | 应用于此规则列表中所有域名的动作 (必填)              | "block", "forward"                   |
| target   | 字符串 | -       | 目标上游组 (当 `action` 为 `forward` 时必须提供)     | 已定义的上游组名称                   |
...
| max_size | 整数   | 1048576 | 远程规则文件的最大大小 (字节), 例如 1048576 表示 1MB | 1 - N (例如 10485760 for 10MB)       |
