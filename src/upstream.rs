                    ...
                    let record = match record_type {
                        RecordType::A => match data.parse::<Ipv4Addr>() {
                            Ok(addr) => {
                                ...
                                let rdata = HickoryRData::A::new(
                                    octets[0], octets[1], octets[2], octets[3],
                                );
                                Record::from_rdata(name.clone(), ttl as u32, RData::A(rdata))
                            }
                            Err(e) => {
                        ...
                        RecordType::AAAA => match data.parse::<Ipv6Addr>() {
                            Ok(addr) => {
                                ...
                                Record::from_rdata(name.clone(), ttl as u32, RData::AAAA(rdata))
                            }
                            Err(e) => {
                        ...
                        RecordType::CNAME => match Name::parse(data, None) {
                            Ok(target) => {
                                ...
                                Record::from_rdata(name.clone(), ttl as u32, RData::CNAME(rdata))
                            }
                            Err(e) => {
